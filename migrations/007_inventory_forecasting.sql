...
CREATE POLICY tenant_isolation_suppliers ON inventory_suppliers FOR ALL USING (company_id = NULLIF(current_setting('app.current_company_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation_forecasts ON inventory_forecasts;
CREATE POLICY tenant_isolation_forecasts ON inventory_forecasts FOR ALL USING (company_id = NULLIF(current_setting('app.current_company_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation_po ON inventory_purchase_orders;
CREATE POLICY tenant_isolation_po ON inventory_purchase_orders FOR ALL USING (company_id = NULLIF(current_setting('app.current_company_id', true), '')::uuid);

DROP POLICY IF EXISTS tenant_isolation_po_items ON inventory_po_items;
CREATE POLICY tenant_isolation_po_items ON inventory_po_items FOR ALL USING (company_id = NULLIF(current_setting('app.current_company_id', true), '')::uuid);
