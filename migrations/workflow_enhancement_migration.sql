...
--
-- NOTE: No REFERENCES companies(id) FKs on new tables.
-- The live companies.id may be INTEGER (SERIAL) while new tables use UUID.
-- company_id isolation is enforced at the application query level.
-- ===============================
...
-- No FK to companies — live DB companies.id type may differ
...
SELECT id::TEXT, 'auto_approve_customer_jobs', 'false'
FROM companies
ON CONFLICT (company_id, setting_key) DO NOTHING;

-- ── 9. SLA configuration table ───────────────────────────────────────────────
-- No FK to companies — company_id stored as TEXT to avoid type mismatch
CREATE TABLE IF NOT EXISTS sla_configs (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id          TEXT NOT NULL,
  max_accept_time     INTEGER DEFAULT 30,
  max_completion_time INTEGER DEFAULT 240,
...
-- ── 15. Update existing customer jobs to pending_approval ────────────────────
UPDATE jobs
...
-- ── 16. Employee profiles — location tracking columns ────────────────────────
